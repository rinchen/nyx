...
### Current (two-level 4k bank mixer hierarchy + cross-block decay + real 4MB LDM window + BWT text trial + JSON stream splitting + 105 tests, all pass)
...
|**JSON stream splitting** (split JSON text blocks into 4 streams — structural `{}[]:,,`, keys, string values, numbers — each independently BWT+MTF+RLE0 encoded and rANS-compressed; 0xFE-prefixed markers in structural stream for unambiguous reconstruction) | json (478KB), big_json (1.1MB), huge_json (5.4MB), nci (32MB) | **improved**: json 0.1% (unchanged, already optimal); nci 20.9%→9.0% (BWT trial now triggers on Binary→Text heuristic); huge_json 2.7% (vs zstd-19 1.8%, zstd-1 2.5% — beats zstd-1 on large JSON); round-trip verified on all files | **kept as default** — METHOD_JSON_SPLIT=7, maps to BlockKind::Text; trial runs on Text blocks ≥256KB; `looks_like_json` heuristic (starts with `{`/`[`, ≥2 colons, quote≥2×colon); structural stream uses 0xFE prefix (can't appear in JSON structural chars); 4-byte LE length prefixes before each sub-stream |
